...
logger.good(`[IZZY] Running IzzyBots Custom v1.1`)
logger.good(`[IZZY] Bots Collect for 1 min ver. `)
...
new WebSocket.Server({
    ...
    userWS = ws
    logger.good('[SERVER] Botter connected!')
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
                    ...
                    logger.good('[SERVER] Starting minions…')
                }
                break
            case 1:
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 30) {
                            ...
                            logger.warn(`[SERVER] Stopping minions in ${30 - seconds} seconds, please wait to ensure no captcha`)
                            seconds++
                        }
                    }, 1000)
                }
    ...
    ws.on('close', () => {
        ...
        logger.error('[SERVER] Botter disconnected!')
	logger.error('[SERVER] Hope you had a good time!')
    })
})
